...
## chapter7 类
### 7.1 定义抽象数据类型
- 引入this：this是一个常量指针，不允许改变this中保存的地址
- （疑问）常量成员函数的作用，为什么要声明为常量
#### 7.1.3 定义类相关的非成员函数
？？什么意思
#### 7.1.4 构造函数
构造函数初始值列表  
...
```
// 此处是将null_book隐式转换为了临时的Sales_data对象
string null_book = "9-999-99999-9";
item.combine(null_book);
```
- 抑制构造函数定义的隐式转换：在要求隐式转换的程序上下文中，可以将构造函数声明为explicit加以阻止  
